  ...
  <body>
    ...
    <?php
      ...
      if (isset($_POST['insert'])) {
          ...
          ********************************************************
          */
          if (empty($isbn) || empty($title) || empty($author) || empty($edition) || empty($publication) || empty($category)) {
              echo "<script>";
              echo "alert('欄位不能為空');";
              echo "history.go(-1);";
              echo "</script>";
          } else {
              $sql = "INSERT INTO book_info (isbn, title, author, edition ,publication, category)
                                      VALUES('$isbn','$title','$author','$edition','$publication', '$category');";
              mysqli_query($db, $sql);
              // echo $result;
          ...
          */
          if (empty($isbn)) {
              ...
              if (mysqli_num_rows($result) == 0) {
                  ...
                  $d_edition = $book_to_delete[3];
                  $d_publication = $book_to_delete[4];
                  $d_category = $book_to_delete[5];
